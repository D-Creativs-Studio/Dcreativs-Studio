...
export function CircularSplitRoll({ services }: CircularSplitRollProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const leftElements = leftItemsRef.current.filter(Boolean) as HTMLElement[];
      const rightElements = rightItemsRef.current.filter(Boolean) as HTMLElement[];
      ...
      const animateSide = (elements: HTMLElement[], arcRad: number, angleOffset: number, depthMax: number, isRight = false) => {
